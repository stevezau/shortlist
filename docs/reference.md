...
| `sync.watch_incremental`                              | `true`                             | read watch history through the local cache. It no longer controls whether the read is incremental — **every sync now reads each library in full**. The incremental read walked by `lastViewedAt:desc` and stopped at the first title older than the cursor, and Plex's own "mark as played" on a series leaves the show row with no `lastViewedAt` at all, so a series marked watched by hand was invisible until the weekly complete read (issue #108). It bought nothing measurable: on a live 47-user, 3-library server a complete sync took 27.4s against 27.3s incremental, because every read fetches a 500-row page per library either way and only 7 of 93 (person, library) pairs held more than one page. A complete read replaces the library, but only when it can prove it saw all of it — it walked the library's reported total. When it cannot (a PMS that reports no total and caps the page), it tops up and deletes nothing, because a truncated read and a mass un-watch look identical from the outside. `false` bypasses the cache and reads straight from the PMS, which also stops `watched_titles` being refreshed — the user page's watched list then goes stale |
| `sync.watch_full_days`                                | `7`                                | how often the dead-library sweep and the un-watch withdrawal run, in days (1–90). It no longer gates the complete re-read — that happens every sync — so a **Plex rating** on an older title, which does not move its `lastViewedAt`, now lands on the very next sync rather than waiting up to a week. The sweep stays on this slower cadence deliberately: it believes a single `/library/sections` response and applies it to every user, so one short answer would erase the cached history of a library that still exists |
