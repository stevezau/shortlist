...
| `privacy.sync_cron`                                   | `15 5 * * *`                       | cron for the nightly privacy sync — a re-merge of every account's share filter. It builds, delivers and promotes nothing, so it can only ever make the server _more_ private; it is the cheapest safety net against drift now that nothing verifies hiding after the fact                                                                                                                                                                                                                                                                                          |
