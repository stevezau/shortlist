...
| `runs.retention`                                      | `3`                                | how many months of run history to keep; after each run, anything older is auto-pruned (runs + per-user traces deleted; picks kept so dashboard metrics survive indefinitely). `0` = keep everything forever                                                                                                                                                                                             |
| `sync.watch_cron`                                     | `""` (daily 04:17)                 | cron expression for the watch-history sync schedule. Blank = built-in default. Set from the Jobs page frequency picker or a custom cron                                                                                                                                                                                                                                                                 |
...
POST /api/auth/pin · GET /api/auth/pin/{id} · GET /api/auth/session · POST /api/auth/logout
     Sign-in is owner-only in BOTH states. Once claimed, the Plex account must match the linked server's owner (403 otherwise). Before it is claimed there is no owner to compare against, so plex.tv is asked directly: an account that owns no Plex server is refused (403) rather than handed a session — someone who merely has a share on your server can never sign in. If plex.tv can't be reached the sign-in fails closed with 503, never open
...
GET  /api/system/jobs?kind=&limit= -> [{id, kind, payload, result, status, attempts, max_attempts, detail, error, created_at, started_at, finished_at}] (background maintenance history, newest first; `kind` narrows it to one job type, which is how the Jobs page shows a single job's own history; runs have their own page)
...
POST /api/system/jobs {kind, payload?, background?} -> the job after an inline drain, or as soon as it is queued when `background` is set (the Jobs page uses that and polls, so a slow job can't end in a proxy timeout that reads as a failure). Only `sync.users`, `sync.history`, `sync.check`, `privacy.sync` and `backup.take` may be triggered by hand — all of them converge-to-desired-state passes that take no target. The rest are queued by the mutation that knows their target and are rejected here with 422: `user.cleanup` (someone turned off), `user.hide`/`user.restore` (paused/un-paused), `row.reconcile` (a row deleted, switched off, narrowed to fewer libraries, or with someone dropped from its audience). Queued work waits for any run in progress, is retried with backoff, survives a restart, and raises a notification if it gives up
