...
| `rows.visibility_cron`                                | `""` (daily 00:00)                 | cron for applying each row's day schedule (`collections.show_days`). Midnight, not 03:30 with the runs: a Monday row would otherwise stay on people's Home until 03:30 on Tuesday, and a row that rebuilds weekly for days. Costs nothing on a night when no row's day turned over — the job compares each row's resolved placement against the state it last applied and makes no Plex or plex.tv calls when they match |
...
rebuilds weekly for days. The job compares each row's resolved placement against the state it last
applied and does nothing at all — no Plex or plex.tv calls — when no row's day has turned over.
