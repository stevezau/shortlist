...
| `sync.watch_cron`                                     | `""` (daily 04:17)                 | cron expression for the watch-history sync schedule. Blank = built-in default. Set from the Schedule page, or the Jobs page frequency picker                                                                                                                                                                                                                                                             |
| `sync.watch_incremental`                              | `true`                             | ask Plex only for titles watched since the last sync (`lastViewedAt>=`) instead of re-reading every watched title, per user, per library, every night. A complete read still happens every `sync.watch_full_days` regardless — an incremental read cannot see an un-watch or a deletion. `false` = always read everything                                                                                 |
...
| `privacy.sync_cron`                                   | `15 5 * * *`                       | cron for the nightly privacy sync — a re-merge of every account's share filter. It builds, delivers and promotes nothing, so it can only ever make the server *more* private; it is the cheapest safety net against drift now that nothing verifies hiding after the fact                                                                                                                                 |
| `sync.check_cron`                                     | `""` (off)                         | cron for the drift check. Off by default: unlike the privacy sync it WRITES corrections to Plex, so running it unattended is a choice to make rather than a default to inherit                                                                                                                                                                                                                           |
