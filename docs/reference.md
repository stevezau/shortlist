...
| `exa.search_type`                                     | `deep-lite`                        | how hard Exa works on each search, trading money for candidates. `instant`/`fast`/`auto` cost $0.007 a search, `deep-lite`/`deep` $0.012, `deep-reasoning` $0.015. `deep-lite` is the default because the cheap modes are erratic: measured on the same two searches, `auto` produced 13 and 8 usable titles against `deep-lite`'s 47 and 36, and once returned nothing at all from 26k characters of page text; `instant` puts a release year on barely one title in nine, which is what Shortlist needs to look a title up. Every search is cached for 14 days and shared across every user on the server, so the extra half-cent buys candidates for the whole roster, not one person. `deep-reasoning` is slower, dearer and scored worse |
...
| `sync.watch_full_days`                                | `7`                                | how often the RECONCILE pass runs, in days (1–90). **Every watch-history read — the nightly job and a run's own top-up alike — reads each library in full**, so this is not what decides whether a complete read happens — it gates the three things that act on a title being ABSENT, and so believe a single response from Plex: dropping cached titles the read no longer returns, sweeping a library removed from the server, and withdrawing pick credit. Before this, the sync read only what had changed since the last pass, ordering by `lastViewedAt` — and a show's own date can lag the episodes it counts, so marking a series watched by hand left it sorted behind the cursor and invisible for up to a week (issue #108). Reading everything costs nothing measurable: 27.4s against 27.3s on a live 47-user, 3-library server, because Plex sends a 500-row page per library either way. A reconcile that would drop more than half a library asks the server a second time first, and keeps the titles if the two answers disagree |
...
| `requests.overseerr.request_as_user_id`               | `0`                                | which Overseerr account requests are filed as. The picker offers the API key's own account and local accounts only — accounts belonging to real people on the server are deliberately left out, because a title is usually wanted by several people while a request has one requester, so choosing a person would put one name on every request for ever. `0` = omit `userId` and let the API key's own account own the request — that account is normally an admin, so its requests auto-approve and go straight to Radarr/Sonarr. Point it at an account without auto-approve to get a second approval gate inside Overseerr. Shortlist never creates the account |
...
GET  /api/report?window=7|30|90|all -> {window, since, first_pick, overall, trend[], per_user[], per_row[], recent[], watch_sync, coverage, runs, requests, top_titles} (what got watched, from picks.watched_at)
     ...
     Un-watching in Plex WITHDRAWS a credit, but only one that Plex's flag was the sole evidence for, and only on the
     periodic reconcile pass (`sync.watch_full_days`). A credit backed by playback we observed — a live session or a play-log entry — is kept: it is a fact
     about a moment, not a mirror of a checkbox, and a partial watch never sets the flag at all, so withdrawing on absence
     would delete the exact signal this feature exists to capture. A sync between reconcile passes withdraws nothing, and
     neither does a read that came back empty for someone.
