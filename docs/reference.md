...
GET  /api/system/jobs -> [{id, kind, payload, result, status, attempts, max_attempts, detail, error, created_at, started_at, finished_at}] (background maintenance history; runs have their own page)
POST /api/system/jobs {kind} -> the job after an inline drain. Only `sync.check` and `privacy.sync` may be triggered by hand; the rest are queued by the mutation or schedule that needs them: `user.cleanup` (someone turned off), `user.hide`/`user.restore` (paused/un-paused), `row.reconcile` (a row deleted, switched off, narrowed to fewer libraries, or with someone dropped from its audience), `sync.users`/`sync.history`/`backup.take` (the scheduled tasks — APScheduler is only the trigger). Queued work waits for any run in progress, is retried with backoff, survives a restart, and raises a notification if it gives up
